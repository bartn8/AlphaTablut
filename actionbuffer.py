    ...
    def store_action(self, board0, board1, reward, weight):
        ...
        with self._lock:
            if action_hash in self.buffer:
                ...
                action[REWARD] = action[REWARD] * (1 - float(weight)) + float(reward) * float(weight)
                if reward == 1:
                    action[WHITE_WINS] +=1
                elif reward == -1:
                    action[BLACK_WINS] +=1
                ...
                action = {CURRENT_STATE: board0.copy(), NEXT_STATE: board1.copy(), VISIT_COUNT: 1, REWARD: float(reward), WHITE_WINS: 0, BLACK_WINS: 0, DRAWS: 0}
